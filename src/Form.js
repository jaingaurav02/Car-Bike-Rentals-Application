...
const Form = () => {
  ...
  const validateFirstName = () => {
    let tempErrors = "";
    if (!formData.firstName) tempErrors = "First Name is required";

    setFirstNameError(tempErrors);
  }

  const validateLastName = () => {
    let tempErrors = "";
    if (!formData.lastName) tempErrors = "Last Name is required";

    setLastNameError(tempErrors);
  }

  const validateUserName = () => {
    let tempErrors = "";
    if (!formData.username) tempErrors = "Username is required";

    setUsernameError(tempErrors);
  }
  ...
  const validateEmail = () => {
    ...
    if (!formData.email) {
      ...
      tempErrors = "Invalid email address";
    }

    setEmailError(tempErrors);
  }

  const validatePassword = () => {
    let tempErrors = "";
    if (!formData.password) {
      tempErrors = "Password is required";
  ...
  const validatePhoneCode = () => {
    ...
    if (!formData.phoneCode) {
      tempErrors = "Country code is required";
    } else if (!isValidCountryCode(formData.phoneCode)) {
      tempErrors = "Invalid country code";
    }

    setPhoneCodeError(tempErrors);
  }

  const validatePhoneNumber = () => {
    let tempErrors = "";
    if (!formData.phoneNumber) tempErrors = "Phone Number is required";

    setPhoneNumberError(tempErrors);
  }

  const validateCountry = () => {
    if (formData.country === "none") alert("Country is required");
  }

  const validateCity = () => {
    if (formData.city === "none") alert("City is required");
  }

  const validatePanNo = () => {
    let tempErrors = "";
    if (!formData.panNo) {
      tempErrors = "PAN Number is required";
    } else if (!isValidPAN(formData.panNo)) {
      tempErrors = "Invalid PAN Number";
    }

    setPanNoError(tempErrors);
  }

  const validateAadharNo = () => {
    let tempErrors = "";
    if (!formData.aadharNo) {
      tempErrors = "Aadhar Number is required";
    } else if (!isValidAadhar(formData.aadharNo)) {
      tempErrors = "Invalid Aadhar Number";
    }

    setAadharNoError(tempErrors);
  };
  ...
  useEffect(() => {
    if (firstNameError === "" && lastNameError === "" && usernameError === "" && emailError === ""
      && phoneCodeError === "" && passwordError === "" && phoneNumberError === "" && panNoError === "" && aadharNoError === ""
    ) {
      const { aadharNo, panNo, firstName, lastName, username, email, country, city, password, phoneCode, phoneNumber } = formData;
      if (aadharNo !== "" && panNo !== "" && firstName !== "" && lastName !== "" && username !== "" && email !== "" && country !== "" && city !== "" && password !== "" && phoneCode !== "" && phoneNumber !== "") {
        setIsDisabled(false);
      } else setIsDisabled(true);
    }
    else setIsDisabled(true);
  }, [formData, aadharNoError, emailError, firstNameError, lastNameError, panNoError, passwordError, phoneCodeError, phoneNumberError, usernameError]);
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          />
          {phoneCodeError && (
            <span className="error-message">{phoneCodeError}</span>
          )}
          <br />
          {phoneNumberError && (
        ...
        <div>
          ...
          {formData.country && (
            <div>
              <label>City:</label>
              <select onBlur={validateCity} name="city" value={formData.city} onChange={handleChange}>
        ...
        <div>
          ...
          />
          {panNoError && (
            <span className="error-message">{panNoError}</span>
          )}
        </div>
        <div>
          <label>Aadhar Number:</label>
          <input
            onBlur={validateAadharNo}
            type="text"
            name="aadharNo"
          ...
          />
          {aadharNoError && (
            <span className="error-message">{aadharNoError}</span>
          )}
        </div>
        <button disabled={isDisabled} style={{ cursor: isDisabled ? 'not-allowed' : 'pointer' }} type="submit">Submit</button>
      </form>
    </div>
  );
};
